...
args = {
    ...
    'method': 'fixed_average',  # or biased, average, fixed biased, fixed average
    'norm': 'linf',  # linf only
...
if args['method'] != 'uniform':
    ...
    model_s = resnet152(pretrained=True).to(device)
    model_s.eval()
    ...
    for image, label in test_loader:
        ...
        while total_q <= args['max_queries']:
            ...
            if ite % 2 == 0 and sigma != ini_sigma:
                ...
                if (rand_loss - loss).item() != 0 and (rand_loss2 - loss).item() != 0:
            ...
            if args['method'] in ['biased', 'average']:
                ...
                while True:
                    prior_loss = F.cross_entropy(
                        ...
                        label)
                    total_q += 1
                    diff_prior = (prior_loss - loss).item()
                    if diff_prior == 0:
                        sigma *= 2
            ...
            if args['method'] == 'average':
                if args['dataprior']:
                    alpha_nes = torch.sqrt(torch.Tensor([A_square * q / (d + q - 1)])).to(device)
                else:
                    alpha_nes = torch.sqrt(torch.Tensor([q / (n + q - 1)])).to(device)
                ...
                if best_lambda < 1 and best_lambda > 0:
                    lmda = best_lambda.cpu().numpy().item()
                else:
            ...
            if not return_prior:
                ...
                if args['method'] == 'average':
                    while True:
                        ...
                        diff_prior = (F.cross_entropy(diff_pred, label) - loss).item()
                        total_q += 1

                        diff_pred = model(
                            adv_image.detach().clone() + sigma * grad)
                        diff_nes = (F.cross_entropy(diff_pred, label) - loss).item()
                        total_q += 1
            ...
            if adv_label != label:
                ...
                print(
                    '\nDone at query:', total_q,
                    'ASR:', success / img_counter,
                    flush=True)
                break
