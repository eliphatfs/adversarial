    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=config).as_default():
            ...
            if FLAGS.method != 'uniform':
                ...
                model_s = ResNet152(
                    source_image_size=model.image_size, use_smoothed_grad=model.use_smoothed_grad)
    ...
    for filename, image in names_images:
        ...
        while total_q <= FLAGS.max_queries:
            ...
            if ite % 2 == 0 and sigma != ini_sigma:
                ...
                rand = rand / \
                    np.maximum(1e-12, np.sqrt(np.mean(np.square(rand))))
                rand_loss2 = model.get_loss(
                    adv_image + ini_sigma * rand, label)
                total_q += 1
            ...
            if FLAGS.method != 'uniform':
                ...
                if FLAGS.show_true and hasattr(model, 'get_grad'):
                    alpha = np.sum(true*prior) / np.maximum(1e-12,
            ...
            if FLAGS.method in ['biased', 'average']:
                ...
                while True:
                    prior_loss = model.get_loss(
                        adv_image + sigma * prior, label)
                    total_q += 1
            ...
            if not return_prior:
                ...
                if FLAGS.method == 'average':
                    while True:
                        diff_prior = (model.get_loss(
                            adv_image + sigma * prior, label) - l)[0]
                        total_q += 1
                        diff_nes = (model.get_loss(
                            adv_image + sigma * grad, label) - l)[0]
                        total_q += 1
                ...
                def print_loss(model, direction):
                    ...
                    for ss in length:
                        les.append(
                            (model.get_loss(adv_image + ss * direction, label) - l)[0])
                    print("losses", les)

                if FLAGS.show_loss:
                    if FLAGS.method in ['average', 'fixed_average']:
                        lprior = model.get_loss(
                            adv_image + lr * prior, label) - l
                        print_loss(model, prior)
                        lgrad = model.get_loss(
                            adv_image + lr * grad, label) - l
                        print_loss(model, grad)
                        lfinal = model.get_loss(
                            adv_image + lr * final, label) - l
            ...
            if FLAGS.show_true and hasattr(model, 'get_grad'):
                if FLAGS.method in ['average', 'fixed_average'] and not return_prior:
                    print("NES angle =", np.sum(true*grad) / np.maximum(1e-12,
                          np.sqrt(np.sum(true*true) * np.sum(grad*grad))))
                print("angle =", np.sum(true*final) / np.maximum(1e-12,
            ...
            if adv_label != label:
                ...
                output_logging.write(
                    filename + ' succeed; queries: ' + str(total_q) + '\n')
                break
        else:
            imsave(os.path.join(FLAGS.output_dir, filename), adv_image)
